...
var WRITING_DATA = "1TpS1a09uQzb_s8dmt9QeotEI-nGbz_9p0dV4R0A_o6A";  // ID to your writing stats spreadsheet
...
function getAlamancText() {
  ...
  var almanac_day = Utilities.formatDate(d, TIME_ZONE, DATE_FORMAT);
  var message;
  var tumblr_message;
  ...
  if (RESCUETIME_TOKEN != "" && RESCUETIME_TOKEN != null) {
    var writingTime = getWritingTime(almanac_day);
    var wordsPerMinute = getWordsPerMinute(almanac_day);
  }
  ...
  if (RESCUETIME_TOKEN != "" && RESCUETIME_TOKEN != null) {
    message = message + "<ul>"
    message = message + "<li>Captured writing time: "  + writingTime + "min";
    message = message + "<ul><li>Words per Minute: " + wordsPerMinute + "</li></ul>";
    message = message + "</ul>"
  }
...
function recalculateGoal(date) {
  ...
  prevMonth = Utilities.formatDate(prevMonth, TIME_ZONE, DATE_FORMAT);
...
function getWritingTime(date) {
  var qs_doc = SpreadsheetApp.openById(WRITING_DATA);
  var sheet = qs_doc.getSheetByName(WRITING_SHEET);
  var time = 0;
  ...
  try {
    time = sheet.getRange(COL_WRITING_TIME + match.getRow()).getValue();
  } catch (e) {
    time = 0; 
  }        
...
function getWordsPerMinute(date) {
  var qs_doc = SpreadsheetApp.openById(WRITING_DATA);
  var sheet = qs_doc.getSheetByName(WRITING_SHEET);
  var wpm = 0;
  ...
  try {
    wpm = sheet.getRange(COL_WRITING_WORDS_MINUTE + match.getRow()).getValue();
  } catch (e) {
    wpm = 0; 
  }        
...
function findDate(value, range) {
  ...
  for (var i = 0; i < data.length; i++) {
    for (var j = 0; j < data[i].length; j++) {
      var tempDate = new Date(data[i][j]);
      var curDate = Utilities.formatDate(tempDate, TIME_ZONE, DATE_FORMAT);  //data[i][j];
      if (curDate == value) {
